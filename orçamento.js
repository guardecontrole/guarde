...
const useHistoryState = (initial) => {
    ...
    // Sem JSON.stringify para evitar erro circular
    const set = (newVal) => { 
...
// --- COMPONENTE PRESET MODAL RESTAURADO (COM VISUAL ESCURO) ---
        ...
        <div className="bg-gray-900 text-gray-200 p-6 rounded-2xl animate-fade-in">
            ...
            <Modal isOpen={!!actionExp} onClose={() => setActionExp(null)}>
                {actionExp && <div className="text-white space-y-2">
                    ...
                    <button onClick={() => { setConfirmDelete(actionExp.id); setActionExp(null); }} className="w-full text-left p-3 hover:bg-gray-700 text-red-400 rounded flex gap-2"><Trash2 size={18}/> Excluir</button>
                </div>}
            </Modal>
        ...
        <div className="space-y-6 pb-24">
            ...
            <div className="bg-gray-800 p-6 rounded-xl shadow-lg">
                ...
                {Object.entries(grouped).map(([grp, grpCats]) => {
                    // Cálculo dos Totais do Grupo (Solicitado)
                    ...
                    return (
                        <div key={grp} className="mb-4">
                            <div className="flex flex-col sm:flex-row justify-between items-center bg-gray-700/30 p-2 rounded mb-2 cursor-pointer" onClick={() => setCollapsed(p => ({...p, [grp]: !p[grp]}))}>
                                <div className="flex items-center gap-2"><Layers size={16} className="text-gray-400"/><span className="font-bold uppercase text-sm tracking-wider text-white">{grp}</span><ChevronDown size={16} className={`transform transition ${collapsed[grp] ? '-rotate-90' : ''}`}/></div>
                                
                                <div className="flex items-center gap-4 mt-2 sm:mt-0 w-full sm:w-auto justify-between sm:justify-end">
                                    {/* TOTAIS DO GRUPO (NOVO) */}
                                    <div className="text-right text-xs mr-2">
                                        <span className="text-gray-400 block sm:inline">Gasto: </span>
                                        <span className="font-bold text-white">{formatCurrency(groupSpent)}</span>
                                        <span className="text-gray-500 mx-1">/</span>
                                        <span className="font-bold text-gray-400">{formatCurrency(groupBudget)}</span>
                                        <span className={`ml-2 font-bold ${groupProgress > 100 ? 'text-red-400' : 'text-green-400'}`}>({groupProgress.toFixed(1)}%)</span>
                                    </div>

                                    <div className="flex gap-2">
                                        <button onClick={e => { e.stopPropagation(); onOpenEditGroupModal(grp); }} className="p-1 hover:text-blue-400"><Edit size={16}/></button>
                                        <button onClick={e => { e.stopPropagation(); onDeleteGroupRequest(grp); }} className="p-1 hover:text-red-400"><Trash2 size={16}/></button>
                                    </div>
                                </div>
                            </div>
